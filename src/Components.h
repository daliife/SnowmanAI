...
struct RigidBody : public Component {
	...
	bool useGravity;	// Gravity force active
	lm::vec3 velocity;	// Initial velocity

	RigidBody() {
		mass = 1.0f;
		useGravity = false;
		velocity = lm::vec3(0, 0, 0);
	}
...
template<> struct type2int<RigidBody> { enum { result = 5 }; };
template<> struct type2int<GUIElement> { enum { result = 6 }; };
template<> struct type2int<GUIText> { enum { result = 7 }; };
